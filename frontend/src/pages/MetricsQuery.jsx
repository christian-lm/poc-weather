...
import { METRIC_OPTIONS, METRIC_UNITS, STAT_OPTIONS } from '../constants/metrics';
...
export default function MetricsQueryPage() {
  ...
  const firstSelectedSensor = queryResults?.results?.[0]?.sensorName || '';
      ...
      <form className="mq-sidebar card" onSubmit={handleSubmit}>
        ...
        <div className="mq-section">
          ...
          <div className="mq-checkboxes">
            {sensors.map(s => (
              <label key={s.id} className="mq-checkbox-label">
                <input
                  type="checkbox"
        ...
        {!queryResults && (
          <div className="mq-empty card">
            ...
            <p style={{ color: 'var(--text-light)', fontSize: '0.85rem' }}>
              Configure the query builder and click "Run Query" to visualize your sensor data.
            </p>
          </div>
        )}
            ...
            <div className="mq-chart-header">
              <div>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                  ...
                  {firstSelectedSensor ? ` — ${firstSelectedSensor}` : ''}
                </p>
              </div>
              ...
              <table>
                ...
                <tbody>
                  {queryResults.results.map(r => (
                    <tr key={r.sensorId}>
                      <td>
                        <strong style={{ color: 'var(--text)' }}>{r.sensorName}</strong>
                        <br />
                      ...
                      <td>
                        <StatusBadge status={Object.values(r.data).some(v => v == null) ? 'timeout' : 'valid'} label={Object.values(r.data).some(v => v == null) ? 'Degraded' : 'Optimal'} />
                      </td>
                    </tr>
                  ))}
                  {queryResults.results.length === 0 && (
                    <tr>
                      <td colSpan={selectedMetrics.length + 2} style={{ textAlign: 'center', color: 'var(--text-light)', padding: '2rem' }}>
